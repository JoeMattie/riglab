...
/** Native v7 fixture: one compound 3D mechanism exercising every element
 * type, BOTH pivot joint kinds, groups, project-level masses, anchor and
 * skeleton bindings, and a named state with vec3 positions. */
export function fixtureProject(): Project {
  return {
    ...
    mechanism: {
      id: 'fixture-project-mechanism',
      name: 'Fixture',
      nodes: [
        { id: 'n1', kind: 'anchor', position: { x: 0, y: 0, z: 0.25 } },
        { id: 'n2', kind: 'free', position: { x: 0.5, y: 0, z: 0.25 } },
        { id: 'n3', kind: 'free', position: { x: 1, y: 0.2, z: 0.3 } },
        { id: 'n4', kind: 'free', position: { x: 1.5, y: 0.1, z: 0.25 } },
        { id: 'n5', kind: 'driven', position: { x: 2, y: 0, z: 0.25 }, channelId: 'ch1' },
        { id: 'n6', kind: 'free', position: { x: 2.5, y: 0, z: 0.2 } },
        { id: 'n7', kind: 'anchor', position: { x: 3, y: 1, z: 0.25 } },
      ],
      elements: [
        {
          id: 'e-link',
      ...
      namedStates: [
        {
          name: 'rest',
          positions: { n2: { x: 0.5, y: 0, z: 0.25 } },
          channelValues: { ch1: 0 },
        },
      ],
    },
    groups: [
      { id: 'grp-1', name: 'boom', elementIds: ['e-link', 'e-bent', 'e-pivot'] },
      {
        id: 'grp-2',
        name: 'drive',
        elementIds: [
        ...
        note: 're-joint needed: transform drive could not be resolved',
      },
    ],
    pointMasses: [
      {
...
/** The v6 fixture VERBATIM as the last per-plane release wrote it (planar
 * mechanisms + assembly instances/bindings) — the input to the v6→v7
 * compound-mechanism migration. Plain JSON on purpose: the v6 shapes no
 * longer have app types. */
export function fixtureProjectV6(): Record<string, unknown> {
  return {
    schemaVersion: 6,
    id: 'fixture-project',
    name: 'Fixture',
