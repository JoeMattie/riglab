...
const HY = 5; // anchor height: the L0 plumb line must clear the floor (slice C)
...
function tautLimitPendulum(): Mechanism {
  const drawnY = HY - Math.sqrt(L0 * L0 - HX * HX); // on the taut circle
  return {
    id: 'taut-limit',
    ...
    gravityOn: true,
    nodes: [
      { id: 'H', kind: 'anchor', position: { x: 0, y: HY } },
...
describe('ACCEPTANCE Phase 3 — rope loaded exactly at its taut limit', () => {
  it('converges (does not creep past the step cap) and reports tension = m·g ±2%', () => {
    ...
    expect(Math.hypot(m.x - 0, m.y - (HY - L0))).toBeLessThanOrEqual(1e-3);
    // rope is at its taut limit: path length == L0 under load
    expect(Math.hypot(m.x - 0, m.y - HY)).toBeCloseTo(L0, 3);
