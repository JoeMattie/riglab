// Pipe-and-fittings model (PLANFILE-quad-workspace slice 3, relocated from
// src/assembly for the v7 dissolution): turns the solved compound mechanism
// into a flat list of solid primitives — true-OD pipe cylinders, fitting
// bodies at realized joints, sleeves/bands/pins for the other realizations —
// for the perspective panel's "Pipe model" render. Composed primitives, not
// CAD: BOM still owns cut math (socket-depth trimming is not modeled).
...
/** Solved world positions per node id — the global solve output (or document
 * positions when idle). */
export type NodeWorld = Record<string, Vec3 | undefined>;
...
/** Build the solid pipe model for the solved compound mechanism. */
export function buildPipeModel(
  ...
  const at = (id: string) => nodeWorld[id];
  ...
  for (const el of mech.elements) {
    if (el.type === 'link' || el.type === 'bentLink') {
      ...
      if (el.maturity === 'engineered' && outer && inner) {
        ...
        pushPipe(a, add(a, scale(dir, reach)), outer.outerDiameterM / 2, false);
        pushPipe(b, sub(b, scale(dir, reach)), inner.outerDiameterM / 2, false);
      } else {
  ...
  for (const el of mech.elements) {
    ...
    const members = memberIds
      .map((id) => {
        ...
        return m ? memberAtJoint(m, nodeId, nodeWorld, pipeOf) : null;
      })
      .filter((m): m is MemberAtJoint => m !== null);
    if (members.length === 0) continue;
    const maxR = Math.max(...members.map((m) => m.radiusM));
    const ghost = members.every((m) => !m.engineered);
  ...
  for (const el of mech.elements) {
    ...
    for (const nodeId of ends) {
      const m = memberAtJoint(el, nodeId, nodeWorld, pipeOf);
      if (!m) continue;
  ...
  for (const [nodeId, members] of nodeMembers) {
    ...
    prims.push({
      ...
      ghost: members.every((m) => !m.engineered),
    });
  }
