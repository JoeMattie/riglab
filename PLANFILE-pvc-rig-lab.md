...
- **3D Assembly mode**: viewport with orbit controls; scene tree (wearer, mechanism instances, point masses, foam); placement gizmos + mirroring (view orientation provides the default plane); binding editor; playback transport drives the full creature; analysis sidebar (mass, CG marker, seesaw balance report); BOM tab with the printable view. *Extended 2026-07-04 by `PLANFILE-quad-workspace.md` (governs where they overlap): all-mechanism synthesis with ghost previews + one-click placement of unplaced mechanisms, a pipe-and-fittings model render toggle, and a quad (Top/Front/Side/Perspective) workspace as a third mode.*
...
*Agreed sequencing (2026-07-04, see DECISIONS.md): Phase 5 ran as two slices. The **examples slice** (done, ahead of Phases 4/4.5 at Joe's direction) delivered the seven example projects as data + the remaining movement clips + the clip-format doc. The **finishing slice** (done, after Phases 4/4.5 landed) delivered the "New from example" menu, onboarding empty-state, printable BOM view, performance pass, keyboard shortcuts, and the visual polish pass; example 7's §9-item-7 deferrals (3D instance placement + the yoke control + head-sweep/jaw-snap control clip) now ship in the full-creature example.*

*Status (2026-07-04): all five phases (0–5, incl. 4.5) complete — acceptance criteria pass as automated tests; CI green (typecheck + lint + test + build) and the built app verified via the scripted `__riglab` hook. Remaining §8.3/§4.4 chrome trims logged as deviations in DECISIONS.md (rotate/driven-instance gizmos, in-3D binding editor, instance-node control mounts) — none affect a §11 acceptance.*
...
- **Keep solver and BOM math under test**: a phase isn't done until its §11 acceptance tests (and the analytic unit cases) pass. Write the tests whenever they help — before, during, or after implementation; test-first is not required. The Playwright smoke test must stay green.
