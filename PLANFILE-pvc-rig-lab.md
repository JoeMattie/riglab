...
- **UI components**: **shadcn/ui** (Radix primitives + Tailwind, components vendored into `src/ui/components/`) for all panel/form/chrome UI — inspector, checklist, materials tables, dialogs, menus, transport controls. Adopted at the start of Phase 3 (the first form-heavy phase); pre-existing chrome migrates opportunistically, fully converged by Phase 5. A **design handoff document** (produced by a separate design pass when the app is feature-complete, ~end of Phase 4) governs the Phase 5 visual polish; until then, use shadcn defaults and avoid bespoke styling.
