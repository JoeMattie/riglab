...
### 6.1 Materials DB
Seed an **editable** table (user can adjust every number; persist overrides in project). Seed with approximate US values and mark them "verify against purchased stock":
...
**Playback transport**: play/pause/scrub bar for the active clip, speed control, and per-clip amplitude sliders, available in both 2D and 3D. During playback the solver runs in kinematic mode each frame (equilibrium overlays optional and off by default during play).
...
- The sketch face intentionally hides: materials, realizations, forces, BOM. Zero forms.
...
- **Bottom panel**: playback transport + clip picker, input channel sliders (with lock toggles), gravity toggle, solver status.
...
7. **Full raptor assembly** — all of the above placed on the wearer with mirrored legs, plus the two arms (single pipe + joint with reel-up rope) and speaker/battery point masses; global CG and BOM populated.
...
- Animation timeline (keyframe input channels, scrub/play) for choreographing a motion.
...
**Phase 5 — Examples + polish.** All seven examples bundled; remaining movement clips (`dance`, `sit down / stand up`, `crouch`, `idle sway`) + documented clip JSON format; onboarding empty-state that opens straight into a silhouette with the pipe tool active and points to examples; printable BOM view; performance pass (drag-solve and clip playback <16 ms on examples); keyboard shortcuts (delete, duplicate, esc, space = play/pause). *Accept: a new user can draw a two-link arm on the silhouette, pivot it, bind it to a hand, and watch it swing during `walk` within their first five minutes and without documentation; the full-creature example opens with working sliders, weight total, and cut list.*
...
- When numeric seed data is uncertain (pipe weights, fitting masses, anthropometry), pick a reasonable value, mark it `approximate: true` in the DB, and surface an "approximate — edit me" badge in the UI rather than guessing silently.
